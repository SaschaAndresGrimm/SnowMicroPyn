...
#uses wx framework to guarantee crossplatform compatibility
###########################################################
...
def openFile(message='Select multiple SMP binarys...'):
    ...
    dialog = wx.FileDialog( None, message=message, wildcard = filters, style = wx.OPEN | wx.MULTIPLE | wx.FD_CHANGE_DIR)
...
class GraphOptions(wx.Dialog):
    ...
    def onApply(self,e):
        ...
        x = self.parent.File[self.parent.current].data[:,0]
        y = self.parent.File[self.parent.current].data[:,1]
        
        if self.auto_x != self.box_autox.GetValue():
            self.parent.axes.set_xlim(np.min(x),np.max(x))
        
        if self.auto_y != self.box_autoy.GetValue():
            self.parent.axes.set_ylim(np.min(y),np.max(y))
...
class SuperPosition(wx.Frame):
    ...
    def OnSaveData(self,e):
        ...
        dlg = wx.FileDialog(
            ...
            defaultFile = filename,
            wildcard = file_choices,
            style = wx.SAVE|wx.OVERWRITE_PROMPT|wx.FD_CHANGE_DIR)
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for line in lines:
                ...
                nl = len(line.get_xdata())
                if length < nl:
                    length = nl
                    
            ndim = np.empty([length,2*len(data)])*np.nan
            
            i = 0
            headerline = ""
            for line in data:
                ndim[:len(line[0]),i] = line[0]
                ndim[:len(line[1]),i+1] = line[1]
                i += 2
                headerline += "x%d [mm]\ty%d [N]\t"%(i/2-1,i/2-1) 
            
            np.savetxt(fname = path, 
                       X = ndim,
                       fmt = '%.3f',
                       delimiter = '\t',
                       newline = '\n',
                       header = 'Automatic written file by SMP Super Position Viewer\n%s'%headerline,
                       comments='# ')
            
            wx.MessageBox('Saved data successfully as %s.' %path, 'Info', 
